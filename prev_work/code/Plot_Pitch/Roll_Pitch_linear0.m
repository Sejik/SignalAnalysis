...
for conditionNum = 3:length(conditions) % 1: ".", 2: ".."
    ...
    for fileNum = 1:length(file)
        ...
        for nameCheckNum = 1:length(columnInfo)
            ...
            if nameChecker
                if strcmp(columnInfo(nameCheckNum), 'Linear Acc(x)')
                    rotationXNum = nameCheckNum;
                end
                if strcmp(columnInfo(nameCheckNum), 'Linear Acc(y)')
                    rotationYNum = nameCheckNum;
                end
                if strcmp(columnInfo(nameCheckNum), 'Linear Acc(z)')
                    rotationZNum = nameCheckNum;
                end
            end
            if strcmp(columnInfo(nameCheckNum), '__Linear Acc(x)')
               rotationXNum = nameCheckNum;
               nameChecker = 0;
            end
            if strcmp(columnInfo(nameCheckNum), '__Linear Acc(y)')
               rotationYNum = nameCheckNum;
               nameChecker = 0;
            end
            if strcmp(columnInfo(nameCheckNum), '__Linear Acc(z)')
               rotationZNum = nameCheckNum;
               nameChecker = 0;
            end
        end
        timeStemp = data.data(1:end, timeStempNum); % 1: TimeStemp
        rotation_x = data.data(1:end, rotationXNum) - 360; % 8: Linear Acc(x)
        rotation_y = data.data(1:end, rotationYNum) - 360; % 9: Linear Acc(y)
        rotation_z = data.data(1:end, rotationZNum) - 360; % 10: Linear Acc(z)
    ...
    xName = strcat(conditions(conditionNum).name, ' Linear Acc(x)');
    yName = strcat(conditions(conditionNum).name, ' Linear Acc(y)');
    zName = strcat(conditions(conditionNum).name, ' Linear Acc(z)');
