...
%% set variable
rawDir = 'C:\Users\user\Desktop\CSV_edit\CSV_edit'; % where the folders of data
...
for conditionNum = 3:length(conditions) % 1: ".", 2: ".."
    ...
    for fileNum = 1:length(file)
        ...
        plot (timeStemp, rotation_y, 'r-');
        plot (timeStemp, rotation_x, 'b-');
    ...
    if length(timeStemp) > length(average_x)
        plot (timeStemp(1:length(average_y)), average_y, 'r-', 'LineWidth', 3);
        plot (timeStemp(1:length(average_x)), average_x, 'b-', 'LineWidth', 3);
        plot (timeStemp(1:length(average_z)), average_z, 'g-', 'LineWidth', 3);
    else
        plot (timeStemp, average_y(1:length(timeStemp)), 'r-', 'LineWidth', 3);
        plot (timeStemp, average_x(1:length(timeStemp)), 'b-', 'LineWidth', 3);
