...
% 0. load data
% 1. preprocessing
    % baseline correction -> artifact rejection by EMD & pearson correlation
% 2. main
    % t-SNE
    % circle regression
% 3. save result
...
for channelNum = 1:size(currentData,2)
    ...
    correctSST = correctSSE + (correctCircleRadius^2)*length(currentData);
    correctR2(channelNum) = correctSSE/correctSST;
    incorrectSSE = sum(incorrectDistance);
    incorrectSST = incorrectSSE + (incorrectCircleRadius^2)*length(currentData);
