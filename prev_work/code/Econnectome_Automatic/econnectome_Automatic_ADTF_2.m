...
% output
pngDir = 'C:\Users\win\Desktop\picture2';
% threhold: range from (max+min)/2 * param.threshold to max
param.threshold = 1; % 2*9/10;
param.axis = 0;
% freq
...
for DataNum = 1:length(eeg_info)
    ...
    for freqNum = 1:length(param.individualFreq)
        for timeNum = 1:length(adtfmatrixs)
            ...
            valmin = min(currentdtfmatrix(currentdtfmatrix~=0));
            if isempty(valmin)
                valmin = 0;
            end
            ...
            options.displimits = [(valmax+valmin)/2*param.threshold+1e-6, 1.0];
