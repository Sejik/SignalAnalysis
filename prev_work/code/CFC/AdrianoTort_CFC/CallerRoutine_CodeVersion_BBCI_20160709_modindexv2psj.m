...
for eegFileNum = 1:n_subjects
    ...
    for comodulogramSize = 1:size(phaseData,2)
        ...
        for ii=1:length(AmpFreqVector)
            ...
            AmpFreq=eegfilt(ampChannel,srate,Af1,Af2, data_length, floor(data_length/3)); % just filtering
            AmpFreqTransformed(ii, :) = abs(hilbert(AmpFreq)); % getting the amplitude envelope
        end
        for jj=1:length(PhaseFreqVector)
            Pf1 = PhaseFreqVector(jj);
            Pf2 = Pf1 + PhaseFreq_BandWidth;
            PhaseFreq=eegfilt(phaseChannel,srate,Pf1,Pf2, data_length, floor(data_length/3)); % this is just filtering
    ...
    individualAverageComodulogram = reshape(individualAverageComodulogram,size(PhaseFreqVector, 2),size(AmpFreqVector, 2));
...
cd(result);
experimentalGroupGrandaverage = reshape(mean(experimentalGroupComodulogram, 1,'omitnan'),size(PhaseFreqVector, 2),size(AmpFreqVector, 2));
controlGroupGrandaverage = reshape(mean(controlGroupComodulogram, 1,'omitnan'),size(PhaseFreqVector, 2),size(AmpFreqVector, 2));
experimentalSubgroupAGrandaverage = reshape(mean(experimentalSubgroupAComodulogram, 1,'omitnan'),size(PhaseFreqVector, 2),size(AmpFreqVector, 2));
experimentalSubgroupBGrandaverage = reshape(mean(experimentalSubgroupBComodulogram, 1,'omitnan'),size(PhaseFreqVector, 2),size(AmpFreqVector, 2));
