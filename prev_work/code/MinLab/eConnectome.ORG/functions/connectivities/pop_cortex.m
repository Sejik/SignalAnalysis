...
valmin = 0; % str2num( get(handles.editmin,'string') );
valmax = 1; % str2num( get(handles.editmax,'string') );
...
elseif isequal(options.dispmodes,'outflow')
    ...
    defaultvalmax = 1; % max(outflows);
    defaultvalmin = 0; % min(outflows);
else
    inflows = sum(dtf.matrix,2)/(size(dtf.matrix,1)-1);
    defaultvalmax = 1; % max(inflows);
    defaultvalmin = 0; % min(inflows);
end

if isnan(defaultvalmin)
    defaultvalmin = 0; % 0.001;
end
if isnan(defaultvalmax)
    defaultvalmax = 1; % 1;
end
valmin = 0; % min(max(defaultvalmin, valmin), defaultvalmax);
valmax = 1; % min(max(defaultvalmin, valmax), defaultvalmax);
...
elseif isequal(options.dispmodes,'outflow')
    outflows = sum(dtf.matrix,1)/(size(dtf.matrix,1)-1);
    valmax = 1; % max(outflows);
    valmin = 0; % min(outflows);
else
    inflows = sum(dtf.matrix,2)/(size(dtf.matrix,1)-1);
    valmax = 1; % max(inflows);
    valmin = 0; % min(inflows);
end
...
mat = dtf.matrix;
valmin = 0; % min(min(mat));
valmax = 1; % max(max(mat));
if isnan(valmin)
    valmin = 0; % 0.001;
end
if isnan(valmax)
...
if isequal(options.dispmodes,'out2in')
    valmin = 0; % min(min(currentdtfmatrix));
    valmax = 1; % max(max(currentdtfmatrix));
