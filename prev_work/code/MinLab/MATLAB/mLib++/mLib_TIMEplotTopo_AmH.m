function [ ] = mLib_TIMEplotTopo_AmH(Potn2D,	PlotTp, CED,				...
									Condi,	TopoTp,							...
...
%	mLib_TIMEplotTopo_AmH(	Data2D, [-500 1500],	...
...
%	mLib_TIMEplotTopo_AmH(	{Data2D_1 Data2D_2}, [-500 1500],	...
%		'/home2/minlab/Tools/MATLAB/EEG_32chan.ced',		...
%		{'USHL' 'MSAD'},	[NaN | 300],		...
%		[ -300 800 ],		'',		[-3 3],		...
%		[-1.5 1.5],			0.9)
...
%	mLib_TIMEplotTopo_AmH({ERP_F___ ERP_U__D ERP__AI_ ERP__A__ ERP__SH_}, ...
...
%		[bbb, aaa]	=	butter(1, [0.5 30]/(eFS/2),'bandpass');
	...
	elseif ~isnan(Filter(1)) % & isnan(Filter(2))		% highpass
		...
		Potn2D		=	filtfilt(bbb, aaa, Potn2D);	% zero-phase: (tp x ep) x ch
		fgFilter	=	HIGH;
	...
	PlotTp			=	[ PlotTp(1) : 1000/eFS : PlotTp(2)-1 ];	% vector
...
%}
	if fgFilter, Potn2D	=	filtfilt(bbb, aaa, Potn2D);	end %zero-phase: 2D or 3D
...
%		Potn2D		=	Potn2D(:, ixTpStart:ixTpFinish, :);	% cond * t * c
		...
		Potn2D		=	permute(Potn2D, [2 1 3]);		% condi*t*c->c*t*condi
	else
