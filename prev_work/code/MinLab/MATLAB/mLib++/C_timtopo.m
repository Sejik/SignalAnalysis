...
% TopoTp: topo용 시간 목록
%		[NaN] : max() 도시
%		[NaN +Inf] (동시 입력) : max(abs()) (+/- 포함 최대) 도시
%		[NaN -Inf] (동시 입력) : min() 도시
%		[+Inf] : maximum variance 도시
%		[-Inf] : minimum variance 도시 (현재 구현 안됨)
%		[] : 생략, topo 출력 안함!
...
%	C_timtopo(	{Data2D_1 Data2D_2}, [-500 1500],	...
%		'/home2/minlab/Tools/MATLAB/EEG_32chan.ced',		...
%		{'USHL' 'MSAD'},	[NaN | 300],		...
%		[ -300 800 ],		'',		[-3 3],		...
%		[-1.5 1.5],			0.9)
...
%	C_timtopo({ERP_F___ ERP_U__D ERP__AI_ ERP__A__ ERP__SH_}, ...
%		[-500 1500], '/home2/minlab/Tools/MATLAB/EEG_30chan.ced',	...
%		{'F___' 'U__D' '_AI_' '_A__' '_SH_'},		...
%		[NaN], [-300 800], '', [0 0], [-1.5 1.5], 0.9)
%
%% 6. real 예시 (5번 기준)
%	C_timtopo({ERP_F___ ERP_U__D ERP__AI_ ERP__A__ ERP__SH_}, ...
%		[-500 1500], '/home2/minlab/Tools/MATLAB/EEG_30chan.ced',	...
...
% ver 0.52 : TopoTp = [ ] 인 경우, topoplot 도시 안함 처리
% ver 0.53 : TopoTp = [NaN] 인 경우, 종전 max(abs()) 에서 max() 로 변경 계산
% ver 0.54 : TopoTp = [NaN +Inf] | [NaN -Inf] 인 경우, max(abs()), min() 계산
...
%% setting %%
	...
	if nargin < 5,	TopoTp		=	NaN;			end	% max 계산
	if nargin < 4,	Condi		=	'';				end

	%% 20160706D. 단, 위 새로운 param 조건에 따라, NaN & Inf 동시 입력시 대응!
	if		any(isnan(TopoTp)) & any(isinf(TopoTp))
		if		TopoTp(isinf(TopoTp)) == Inf
			minmax		=	3;							%max(abs())(+/-포함 최대)
		elseif	TopoTp(isinf(TopoTp)) == -Inf
			minmax		=	2;							% min() 도시
		end

		TopoTp			=	NaN;
	elseif	any(isnan(TopoTp))
		minmax			=	1;							% max() 도시
		TopoTp			=	NaN;
	else
		minmax			=	4;							% any number
...
%}
	...
	if		any(isnan(TopoTp))
		TopoTp			=	NaN;
	elseif ~isempty(TopoTp)								% [ ] 경우는 처리 안함
		TopoTp			=	ones(1, size(Potn2D,3)) * TopoTp(1);
	end
...
%}
	timtopo(Potn2D,			CED,											...
			...
			'tradeoff',		DispRatio,										...
			'minmax',		minmax);		% 1==max, 2==min, 3==minmax
