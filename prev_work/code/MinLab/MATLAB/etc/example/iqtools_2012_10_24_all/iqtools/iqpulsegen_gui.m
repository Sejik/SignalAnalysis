...
% Last Modified by GUIDE v2.5 22-Sep-2012 09:19:15
...
set(handles.popupmenuDownload, 'TooltipString', sprintf([ ...
    ...
    '"Data to Ch1", "Data to Ch2" and "Data to Ch1+Ch2" means that the\n' ...
    'generated signal is downloaded into Ch1 only, Ch2 only or Ch1 and Ch2\n' ...
    'respectively.  In Digital Up-conversion modes, the calculated signal\n' ...
    'is used as I and Q (i.e. I=Q) and downloaded into the selected channel(s)\n' ...
    'This can be used to load the same or different signals into the\n' ...
    'channels of the AWG.']));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1)
    set(handles.editPWSamples, 'String', sprintf('%d ', round(value .* sampleRate)));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1)
    set(handles.editRiseSamples, 'String', sprintf('%d ', round(value .* sampleRate)));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1)
    set(handles.editFallSamples, 'String', sprintf('%d ', round(value .* sampleRate)));
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[pulse rpt sampleRate] = calcPulse(hObject, eventdata, handles);
...
iqdownload(pulse, sampleRate, 'downloadToChannel', downloadToChannel, ...
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1e9 && isequal(floor(value),value))
    set(handles.editFallSec, 'String', sprintf('%g ', value / sampleRate));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1e9 && isequal(floor(value),value))
    set(handles.editRiseSec, 'String', sprintf('%g ', value / sampleRate));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1e9 && isequal(floor(value),value))
    set(handles.editPWSec, 'String', sprintf('%g ', value / sampleRate));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1)
    set(handles.editOffSamples, 'String', sprintf('%d ', round(value .* sampleRate)));
...
if (isvector(value) && min(value) >= 0 && max(value) <= 1e9 && isequal(floor(value),value))
    set(handles.editOffSec, 'String', sprintf('%g ', value / sampleRate));
...
if (isvector(pw) && isvector(rise) && isvector(fall) && isvector(off) && isvector(low) && isvector(high))
    ...
    set(handles.editPRISec, 'String', sprintf('%g', priTime));
    set(handles.editPRISamples, 'String', sprintf('%d', pri));
    if (priTime ~= 0)
        set(handles.editFrequency, 'String', sprintf('%g', 1/priTime));
    end
end
...
function [pulse rpt sampleRate] = calcPulse(hObject, eventdata, handles)
%try
sampleRate = evalin('base',get(handles.editSampleRate, 'String'));
pw = evalin('base', ['[' get(handles.editPWSamples, 'String') ']']);
rise = evalin('base', ['[' get(handles.editRiseSamples, 'String') ']']);
...
if(FileName~=0)
    [Y rpt sampleRate] = calcPulse(hObject, eventdata, handles);
    XDelta = 1/sampleRate;
    XStart = 0;
    InputZoom = 1;
    try
