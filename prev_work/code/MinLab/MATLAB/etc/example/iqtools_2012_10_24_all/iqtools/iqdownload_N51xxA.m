function f = iqdownload_N51xxA(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, downloadToChannel, sequence)
% Download an IQ waveform to Agilent Vector Signal Generators...
% 'marker1', 'marker2', 'segmNum' inputs are not used
% in the function. They are here to maintain consistency with the other
% functions

% v2.0 - Adarsh Narasimhamurthy
...
switch (downloadToChannel{1})
    % Note that both channel1 & 2 need to have waveforms. See C++ example comments
    case 'I+Q to channel 1+2'
        downloadSignal(f, data.', ArbFileName, fs, fCenter, amplitude);
    case 'I+Q to channel 2+1'
        downloadSignal(f, complex(imag(data.'), real(data.')), ArbFileName, fs, fCenter, amplitude);
    otherwise
        errordlg('MXG download requires I+Q data. Please keep the option at I+Q to channel 1+2 or 2+1')
end
...
fprintf(deviceObject, ':SOURce:RADio:ARB:RSCaling 100');
...
%    fprintf('cmd = %s\n', s);
    ...
    if (~strncmp(lower(result), '+0,no error', 10) && ~strncmp(lower(result), '+0,"no error"', 12))
