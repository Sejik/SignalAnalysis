...
% Radar simulation with a moving target
...
numSteps = 50;
% interpolation method for flight path ('linear', 'cubic', 'spline')
...
% amplitude ratio
amplRatio = amplRatioMax * (dist - min(dist(1:end))) / (max(dist(1:end)) - min(dist(1:end)));
...
if (~isempty(ax))
    ...
    hold(ax(1), 'on');
    plot(ax(1), radarPos(:,1), radarPos(:,2), 'k*');
...
pSamples = ceil(ceil(pTime * fs) / granularity) * granularity;
...
%    idata = real(iqdata .* exp(j*2*pi*(1:pSamples)*flo/fs));
    idata = real(iqdata);
...
%    grid(ax(4), 'on');
    ...
    if (downloadToAWG)
        if (numRecv == 1)
            iqdownload(iqdata, fs, 'segmentNumber', i, 'keepOpen', 1, 'run', 0, 'downloadToChannel', 'I to channel 1');
        elseif (numRecv == 2)
            iqdownload(complex(idata(1,:), idata(2,:)), fs, 'segmentNumber', i, 'keepOpen', 1, 'run', 0, 'downloadToChannel', 'I+Q to channel 1+2');
        end
    end
    advanceMode = 'Auto';
    ...
    seq(2*i-1).sequenceLoops = floor(deltaTime/pri); % loop
    seq(2*i-1).sequenceAdvance = advanceMode;    % seq adv
    % the pause segment
    seq(2*i-0).segmentNumber = 0;      % idle segment
    seq(2*i-0).segmentLoops = floor(pri * fs) - pSamples; % idle duration
...
if (downloadToAWG)
    ...
    seq(2*numSteps).scenarioEnd = 1;
    % fprintf('downloading sequence...\n');
    f = iqopen();
    if (numRecv == 1)
        iqseq('define', seq, 'keepOpen', 1, 'run', 0, 'downloadToChannel', 'I to channel 1');
        % switch to "scenario" mode and run
        fprintf(f, ':abort1');
        fprintf(f, ':func1:mode stsc');
        fprintf(f, ':init:imm1');
    else
        iqseq('define', seq, 'keepOpen', 1, 'run', 0, 'downloadToChannel', 'I+Q to channel 1+2');
        % switch to "scenario" mode and run
        fprintf(f, ':abort');
        fprintf(f, ':inst:coup:stat on');
        fprintf(f, ':func1:mode stsc');
