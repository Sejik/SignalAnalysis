function driver = iqdownload_N824xA(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, downloadToChannel, sequence)
...
% v1.0 - Vinod Cherian, MathWorks
    ...
    switch (downloadToChannel{1})
        ...
        case 'I+Q to channel 1+2'
            gen_arb_N824x(arbConfig, driver, 1, real(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, imag(data), marker2, fs, segmNum);
        case 'I+Q to channel 2+1'
            gen_arb_N824x(arbConfig, driver, 1, imag(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, real(data), marker2, fs, segmNum);
        case 'I to channel 1'
            gen_arb_N824x(arbConfig, driver, 1, real(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, real(data), marker1, fs, segmNum);
        case 'I to channel 2'
            gen_arb_N824x(arbConfig, driver, 1, real(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, real(data), marker1, fs, segmNum);
        case 'Q to channel 1'
            gen_arb_N824x(arbConfig, driver, 1, imag(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, imag(data), marker1, fs, segmNum);
        case 'Q to channel 2'
            gen_arb_N824x(arbConfig, driver, 1, imag(data), marker1, fs, segmNum);
            gen_arb_N824x(arbConfig, driver, 2, imag(data), marker1, fs, segmNum);
    end
