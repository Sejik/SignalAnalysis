...
timelist{1,1}=[51:201];     % alpha2의 Timewindow. -400ms ~ -100ms
% timelist{3,1}=[251:501];    % beta1의 Timewindow. 0ms ~ 500ms
% timelist{4,1}=[251:751];    % beta2의 Timewindow. 0ms ~ 1000ms
% timelist{5,1}=[251:501];    % theta의 Timewindow. 0ms ~ 500ms
...
% chanlist{3,1}=[24, 25, 26]; 
% chanlist{4,1}=[23,24,26,27,29,31];
% chanlist{5,1}=[23,24,26,27]; 
    ...
    for trialnumb= 1:length(trialname)
        for datanumb= 1:length(dataname)
            for subnumb= 1:length(subname)
                ...
                Mean_list(subnumb,1)= AN;
                % Peak_list(subnumb,3)= peak_time_ms;   % 시간(ms)단위로 변환한 Latency 결과값.
                
                clear ERP_filt_bl TFe_bl TFi MaxTime MaxValue Max_All Max_Chan Max_Peak list1 list2 peakBuf_list peakBuf_list_max peak_mean peak_time Potential_Buf potential_topo A AA
                close all
                
            end
            % 조건별 피험자 개개인의 데이터를 저장합니다.
            % 참고용으로, 최종적으로 사용하는 테이블은 아닙니다.
            eval(['FILENAME=''skk_PlotPeak_' char(ananame{ananumb}) '_MaximumPeakAveraged_' char(freqname{freqnumb}) '_' char(trialname{trialnumb}) '_' char(dataname{datanumb}) ''';']);
            cd Peak;
            save(FILENAME, 'Peak_list');
            eval(['FILENAME=''skk_PlotPeak_' char(ananame{ananumb}) '_MeanPeakAveraged_' char(freqname{freqnumb}) '_' char(trialname{trialnumb}) '_' char(dataname{datanumb}) ''';']);
            save(FILENAME, 'Mean_list');
            cd ..;
            clear Peak_list
...
for freqnumb=1:length(freqname)
    ...
    for trialnumb=1:length(trialname)
        for datanumb=1:length(dataname)
            ...
            cd Peak;
            eval(['load(''skk_PlotPeak_' char(ananame{ananumb}) '_MaximumPeakAveraged_' char(freqname{freqnumb}) '_' char(trialname{trialnumb}) '_' char(dataname{datanumb}) '.mat'');']);
            cd ..;
    ...
    eval(['FILENAME=''skk_TotalPeak_' char(ananame{ananumb}) '_MaximumPeakAveraged_' char(freqname{freqnumb}) ''';']);
    cd Peak_total
