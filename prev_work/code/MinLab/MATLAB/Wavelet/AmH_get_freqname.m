%%===============================================================================
function [ FreqName ] = AmH_get_freqname(f)	%벡터 경우 vs cell array 경우 구분!
% detect band of array f, and return band name
	% 먼저 타입을 검사하여 처리방법을 결정함
	if isfloat(f)
		FreqName			=	AmH_get_freqname_vector(f);

	elseif iscell(f)
		FreqName			=	f;			%f와 같은 규격

		for ff = 1 : length(f)
			FreqName{ff}	=	AmH_get_freqname_vector(f{ff});
		end
	end

function [FreqName] = AmH_get_freqname_vector(f)
