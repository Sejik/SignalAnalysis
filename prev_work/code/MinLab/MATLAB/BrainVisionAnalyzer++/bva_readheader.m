...
for section = 1:length(sections) - 1
    ...
    switch fieldname
        ...
        case {'channelinfos' 'coordinates' 'markerinfos'}
			% 20160618A. tigoum:
			%	if first index of param will be started at 0, then shift.
            for line = sections(section) + 1:sections(section + 1) - 1
                [parameter, value] = strread(raw{line}, '%s%s', 'delimiter','=');
				if line==sections(section)+1 & str2double(parameter{1}(3:end))<=0
					shift = abs(str2double(parameter{1}(3:end)))+1;
				end

                hdr.(fieldname)(str2double(parameter{1}(3:end))+shift) = value;
            end
        case 'comment'
