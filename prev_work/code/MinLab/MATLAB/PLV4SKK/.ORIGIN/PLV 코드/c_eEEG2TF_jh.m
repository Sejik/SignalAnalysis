...
freqs=[8:1/2:13];  % 8~13 즉, 알파 파를 잡는 다. step 0.5
chs=[1:32]; % 채널
...
selchanname={'P3', 'Pz', 'P4'}; % 지금은 사용을 안해요
...
for datanumb= 1:length(dataname)
    for trialnumb=1:length(trialname)
        for subnumb= 1:length(subname)
            ...
            bl=zeros(size(eEEG,2));
            % bl은 에폭의 크기를 잡아줍니다.
            % 틀을 만들어 주는 과정 (zero는 다 0으로 채운 행렬을 만드는 것, eEEG의 3차원 데이터 중 2차원 데이터의 크기 만큼)
            % baseline을 만드는 것: 자극 전까지의 값들을 평균 내어서 baseline을 맞춰줌. correction을 한다. common reference 같은 방법이 있음.
            ...
            % 
            bl=single(bl); % single: 무언가 데이터를 가공하거나 빠르게 하는??, 단정밀도로 바꾸어줌
            ...
            for ii=1:length(chs);
                   ch=chs(ii);
                   bl(1,:)=squeeze(mean(eEEG([1:250],:,ch),1));            %자극제시 전 -500~0ms 로 baseline 을 설정함. 1개 층인 데이터를 차원을 줄여서 넣어주는 것에 사용함
