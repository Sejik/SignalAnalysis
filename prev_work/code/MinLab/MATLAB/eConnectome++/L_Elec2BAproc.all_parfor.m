...
%	options.step		=	round(EEG.points/100);		% '1%' step	%-[
	options.step		=	round((FinishPoint - StartPoint + 1));	% all size
...
%%				WorkStart, WorkEnd, nTP, WorkEnd/nTP*100);
	Prgs				=	TimedProgressBar( NumWorkers, 40-5,				...
								'wait for ', ', completed ', 'Concluded in ' );
%	pause(0.01);									% Replace with real code
...
%		electrodesV		=	EEG.data(options.vidx, ix +WorkStart -1); % ch x tp
		electrodesV		=	cEEG{ix}(o_vidx);	% tp series 내부에 ch 묶음
%		electrodesV		=	cEEG(:, ix);			% tp series 내부에 ch 묶음
...
%					ix / nTP *100, ix, nTP);
		Prgs.progress;								% Also percent = p.progress;
	end		% parfor
		Prgs.stop;									% Also percent = p.stop;
%________________________________________________________________________________
...
function [] = S_showStatus(Percent, timeSec, barWid, marker)

	persistent	LastLength;

	if nargin < 4, barWid = 39; end
	if nargin < 3, marker = '>'; end
	if nargin < 2, timeSec= 0; end
	...
	WaitMsg				=	'Wait for ';
	DoneMsg				=	', completed ';
	Format				=	[ '%03d:%02d:%04.1f'			...		% hh:mm:ss.s
								DoneMsg '%5.1f%%'];	% 6 char wide, percentage
	if Percent == 0 & timeSec == 0
		timedMsg		=	sprintf('---:--:--.-%s%5.1f%%', DoneMsg, Percent);
	else
		hh				=	fix( timeSec / 3600 );
		mm				=	fix( ( timeSec - hh * 3600 ) / 60 );
%		ss				=	max( ( timeSec - hh * 3600 - mm * 60 ), 0.1 );
		ss				=	fix( ( timeSec - hh * 3600 - mm * 60 ) );
		timedMsg		=	sprintf( Format, hh, mm, ss, Percent );
	end
    statusMsg			=	[ WaitMsg timedMsg ];

	% 진행과정 표기할 bar 길이를 계산하자.
	x					=	round( Percent * barWid / 100 );
	if x < barWid
	...
	if Percent > 0 && Percent < 100
		cursorRewinder= repmat( char(8), [1], [0+LastLength] );
%		disp( [ cursorRewinder, statusLine ] );
		fprintf(stdout, '%s', [ cursorRewinder, statusLine ] );
	elseif Percent == 100
		cursorRewinder= repmat( char(8), [1], [0+LastLength] );
%		disp( [ cursorRewinder, statusLine ] );
		fprintf(stdout, '%s', [ cursorRewinder, statusLine ] );
