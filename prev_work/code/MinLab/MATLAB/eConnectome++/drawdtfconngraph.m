...
% ==========================================
%
% work modified by tigoum 2016/06/17
%	- addition with 4th parameter : handle of img, it is used to instead of cmap
%
if nargin < 4, cdata = []; end
% ==========================================
...
elseif isequal(inopt.Channels,'single')
	...
	DispConnectivity(rr, cc, dtfVal, roiPos, inopt);		% re-coded by tigoum
else
	arrayfun(@(rr)arrayfun(@(cc) DispConnectivity(rr,cc, dtfVal,roiPos,inopt),...
							[1:nRois]), [1:nRois]);			% re-coded by tigoum
end

function [] = DispConnectivity(rr, cc, dtfVal, roiPos, inopt)	% local func %-[
	% maked by tigoum
	% -----
	dispmin	=	inopt.ValLim(1);
	dispmax	=	inopt.ValLim(2);
	maxmin	=	dispmax-dispmin;							% added by tigoum

	% mat(~mat)=nan;
%	valmin	=	min(min(mat));
	valmin	=	min(dtfVal( 0 < dtfVal ));					% 0 제외한 최소값
	valmax	=	max(dtfVal(:));
	%maxmin	=	valmax-valmin;								% remark by tigoum
	if ~maxmin, maxmin = 1.0; end

	% do not display connectivity values outside the limit.
	if dtfVal(rr, cc)<dispmin -1e-6 | dtfVal(rr, cc)>dispmax + 1e-6, return; end
	% do not display zero connectivity values.
	if dtfVal(rr, cc) <= 0.0, return; end

	% -----
	ArrowSizeLimit = inopt.ArSzLt;

	nRois	=	size(dtfVal, 1);
	arcH	=	-ones(nRois);	%== -ones(nRois, nRois);
	arwH	=	-ones(nRois);	%== -ones(nRois, nRois);

	cmap	=	colormap;
	cnum	=	size(cmap,1);

	% -----
%	normVal	=	(dtfVal(rr, cc) - valmin)/maxmin;			% 화살표 굵기 정규화
	normVal	=	(dtfVal(rr, cc) - dispmin)/maxmin;			% 화살표 굵기 상대화
	normVal	=	min(max(0, normVal), 1);
	siz		=	ArrowSizeLimit(1) + diff(ArrowSizeLimit) * normVal;

%	hImg	=	findobj('tag', 'image');				% tigoum
	colIDX	=	round(normVal*(cnum-1)+1);
	col		=	cmap(colIDX,:);

	% from output 'cc' to input 'rr'
	hold on;
	[arcH(rr, cc) arwH(rr, cc)] = drawpipe(roiPos(cc, :), roiPos(rr, :), siz);
	set([arcH(rr, cc); arwH(rr, cc)], 'faceColor', col);	% flow line color !
	return	% DispConnectivity	%-]
...
wasHold=ishold;
pipeH=surf(pipes.X, pipes.Y, pipes.Z, 'EdgeColor', 'none', 'FaceLighting','phong');	%, 'FaceColor','r');
% material default;
% lighting phong;
hold on
arrowH=surf(arrow.X, arrow.Y, arrow.Z, 'EdgeColor', 'none', 'FaceLighting','phong');	%, 'FaceColor','r');
% material default;
% lighting phong
if ~wasHold
   hold off
