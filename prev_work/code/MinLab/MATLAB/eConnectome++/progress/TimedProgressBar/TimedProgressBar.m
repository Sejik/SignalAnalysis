classdef TimedProgressBar < handle
    ...
    methods
		...
		function obj= TimedProgressBar( targetCount, barWidth, waitMsg, ...
			...
			obj.percentDoneMsg '%5.1f%%']; % 4 characters wide, percentage
			obj.textWidth = length(obj.waitMsg) + 3 + 1 + 2 + 1 + 4 + ...
							length(obj.percentDoneMsg) + 1 + 4 + 1;
			%            fprintf(1, [ '\n' repmat( ' ', 1, 20 ) '\n' ] );  % buffers multitasking uncertanties of the output to the command line
			obj.showStatus( 0, [ repmat( ' ', 1, obj.textWidth-6 ),'  0.0%' ] );
		end
		...
		function percent= progress(obj)
			...
			if (percent * 10) ~= floor(percent * 10), return; end %find olny xx.x
    ...
    methods  ( Access= protected )
		...
		function showStatus( obj, percent, statusMsg )
			...
			if percent > 0 && percent < 100
				cursorRewinder= repmat( char(8), 1, 0+obj.rewindLength );
				disp( [ cursorRewinder, statusLine ] );
%				fprintf(1, '%s', [ cursorRewinder, statusLine ]);
			elseif percent == 100
				cursorRewinder= repmat( char(8), 1, 0+obj.rewindLength );
				disp( [ cursorRewinder, statusLine ] );
%				fprintf(1, '%s', [ cursorRewinder, statusLine ]);
