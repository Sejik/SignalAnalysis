...
%}
	...
	reg_param	=	reg_param(end) .* ( ratio .^ [npoints-1:-1:0] );
	xi			=	repmat(xi, 1, length(reg_param));
	beta		=	repmat(beta, 1, length(reg_param));
	s2			=	repmat(s2, 1, length(reg_param));	% expand col->row
	reg_param2	=	repmat(reg_param.^2, size(s2,1),1);	% expand row->col
	f			=	s2 ./ (s2 + reg_param2);
	fxi			=	f .* xi;
	fbeta		=	(1-f) .* beta(1:p, :);
...
if (locate)
  [reg_corner,rho_c,eta_c] = l_corner2(rho,eta,reg_param,U,sm,b,method);
end
