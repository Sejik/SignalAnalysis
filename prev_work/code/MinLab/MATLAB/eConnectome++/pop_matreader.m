function EEG = pop_matreader( name, pathstr, WishLabels )
...
if nargin < 3, WishLabels = []; end					% 20160702A. added by tigoum
...
%	eeg.{
%		eCHN: {1x32 cell}
%		eEEG: [3500x32x60 double] or [3500x60x32 double] <- check must it!
%		eFS: 500
%		eMRK: [1x60 double]
%	}
	EEG				=	struct;
...
%	eeg.{
%		ERP_11: [1000x30 double]
%		EVK_21: [280x1000x30 double]
%		TOA_12: [280x1000x30 double]
%		TOT_22: [280x1000x30 double]eMRK	: marker
%	eeg.{
...
if ~isempty(WishLabels)				% 20160702A. added by tigoum
	ixLive		=	find( ismember(EEG.labels, WishLabels) );%살아있는 채널만
	ixRemv		=	find(~ismember(EEG.labels, WishLabels) );%죽어있는 채널만
	EEG.data	=	EEG.data(ixLive, :);
	EEG.labels	=	EEG.labels(ixLive);
