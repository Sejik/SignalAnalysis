...
toolhandle = findobj(hButtons,'tag','Plottools.PlottoolsOn'); % by tigoum
set(toolhandle,'Visible','on');
...
%	valmin = min(min(dtf.matrix));
	valmin = min( dtf.matrix ( 0 < dtf.matix ) );
...
%{
%valmin = min(min(dtf.matrix));
valmin = min( dtf.matrix( 0 < dtf.matrix ) );
valmax = max(max(dtf.matrix));
if isnan(valmin)
    valmin = 0.001;
end
...
% display DTF graphics.
axes(handles.sceneaxes);			% sceneaces 에 cortex + connectivity drawing
cla;								% axes 지우기 (옵션에 따라 다시 그리기 위해)
hold on;
displayDTFG(model, options, dtf);	% re drawing
...
% 20160524 by tigoum, begin
[valmin, valmax] = deal(min(valmin, valmax), max(valmin,valmax));
[valmin, valmax] = deal(max(valmin, 0), min(valmax, 1));
if valmin == valmax, [valmin, valmax]= deal(valmin-1.0e-3, valmax+1.0e-3); end
[valmin, valmax] = deal(max(valmin, 0), min(valmax, 1));
% 20160524 by tigoum, fin
...
%% ========================================================================
%% display directed transfer function matrix.
axes(handles.imageaxes);					% added by tigoum
hold on;									% added by tigoum
caxis(options.minmax);						% added by tigoum
%colorbar;
...
elseif isequal(options.dispmodes,'outflow')
    outflows = sum(dtf.matrix,1)/(size(dtf.matrix,1)-1);
%    valmin = min(outflows);
    valmin = min(outflows ( 0 < outflows ) );
    valmax = max(outflows);
else
    inflows = sum(dtf.matrix,2)/(size(dtf.matrix,1)-1);
%    valmin = min(inflows);
    valmin = min(inflows ( 0 < inflows ) );
    valmax = max(inflows);
end
...
if options.iscortex							% draw cortex half sphere
	...
	if isequal(options.type,'ROI')			% color is cover to ROI on cortex
		for i=1:nroi
			roi_vert_idx = model.vertices{i};
			cortexFaceVertexCData(roi_vert_idx, :) =	...
				repmat(cmap(i,:), length(roi_vert_idx),1);
		end
	end

	V	=	model.cortex.Vertices;			% 20516 x 3
	F	=	model.cortex.Faces;				% 41136 x 3
%	c	=	model.cortex.FaceVertexCData;	% 41136 x 3
	C	=	cortexFaceVertexCData;			% 20516 x 3
%	V(10:10000,:)	=	0;

	% display the cortex with different ROIs having different colors.
	patch('SpecularStrength',0,	'DiffuseStrength',0.8,...
	'FaceLighting',			'phong',	...
	'Vertices',				V,			...	%model.cortex.Vertices,...
	'LineStyle',			'none',		...
	'Faces',				F,			...	%model.cortex.Faces,...
	'FaceColor',			'interp',	...
	'EdgeColor',			'none',		...
	'FaceVertexCData',		C); 			%cortexFaceVertexCData,	...
	% -----
	alpha(0.10);	% 20160523 by tigoum % control transparency skin & cortex
	% -----
end
...
if options.dtf							% draw the connectivity flow!!!
...
%    valmin = min(min(currentdtfmatrix));
    valmin = min( currentdtfmatrix( 0 < currentdtfmatrix ) );
...
%cmin = min( currentdtfmatrix( 0 < currentdtfmatrix ) );
...
%
% 20160624A. 화면에 표시되는 min, max limitation 을 정밀조정하고 색상 반영
%
% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
function SophisticateAxes(handles)
...
%    valmin = min(min(dtf.matrix));
    valmin = min( dtf.matrix( 0 < dtf.matrix ) );
    valmax = max(max(dtf.matrix));
    [row col] = find(dtf.matrix==valmax, 1);
    options.whichchannel = [row col];
elseif isequal(options.dispmodes,'outflow')
    outflows = sum(dtf.matrix,1)/(size(dtf.matrix,1)-1);
%    valmin = min(outflows);
...
if valmin==valmax,[valmin valmax]=deal(valmin-valmin/1e+9,valmax+valmax/1e+9);end
% 같은 값이면, caxis 에서 에러남
...
%%	options.displimits = [valmin, valmax];
%end
set(handles.options, 'userdata',options);				% re-register
...
imgh		=	imagesc(plotimage,'tag','image');
% 20160622A. 적용
Over		=	find( valmax < dtf.matrix );			% vector 형 index
[ovX ovY]	=	ind2sub(size(dtf.matrix), Over);
% -----
Under		=	find( dtf.matrix < valmin );			% vector 형 index
[unX unY]	=	ind2sub(size(dtf.matrix), Under);
% -----
%{
CData		=	get(imgh, 'CData');
CData(ovX, ovY, :)		=	[255,255,255];				% white
CData(unX, unY, :)		=	[  0,  0,  0];				% black
set(imgh, 'CData',CData);
%}
%nroi = size(dtf.matrix,1);
for i=0:nroi
    lx = [0.5 nroi+0.5];
