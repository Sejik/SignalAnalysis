...
%}
B(:,:,2:p+1)	=	reshape(A, nchan,nchan,p);			% MVAR matrix

%% CalcDTF(MVAR, low_freq, high_freq, p, fs);
...
%{
	for i				=	1:nchan
		for j			=	1:nchan
			...
			Af(i,j)	=	Af(i,j) - sum( B(i,j,:).*weight );
		end
	end
%}
	w			=	[];
	w(1,1,:)	=	exp(-pi*fre*dt*[0:p]*2i);			% h=[1:p+1], h-1=[0:p]
	w			=	repmat(w, nchan, nchan, 1);			% extend
	Af			=	Af - sum(B.*w, 3);
...
%			Apf(:,i)		=	[];
%			Apf(j,:)		=	[];
			Apf				=	Af([1:j-1 j+1:nchan], [1:i-1 i+1:nchan]);
			det2			=	det(Apf);
%			det2			=	det2.*conj(det2);
			det2			=	abs(det2.^2);
...
%}
SUM				=	repmat(sum(theta2, 2), 1, nchan, 1); % extend to j dim
gamma2			=	theta2 ./ SUM;
