...
try
	EEG	=	eeg_checkset(EEG);						% 호출: data 구조 조정 됨
%{
	EEG =	% struct의 구성 목록	%-[
	setname: ''
	filename: ''
	...
	condition: ''
	session: []
	comments: 'Original file: EMO_NEW_su0024_Artifact Rejection_BNH.dat'
	nbchan: 2
	trials: 22
	pnts: 1250
	srate: 500
...
%}
	...
	% 20160603A. tigoum: BV 에서 segm 후 export 된 dat 는 3-D 로 epock 구분 됨
	%	BUT, PACT는 2-D만 지원하므로, 3-D 데이터의 구조를 조정할 필요성 !
	% 1. epoch을 concatenate 하는 방법
	% 2. epoch을 average 하는 방법
	% 참조할 EEG.[] 의 구성요소
	%	-> times: [1x1250 double]
	%	-> nbchan: 2
	%	-> trials: 22
...
if ndims(EEG.data) == 3
	...
	Size	=	size(EEG.data);						% 각 차원의 크기
	sSize	=	strjoin( arrayfun(@(x) num2str(x), Size), ' x ' );
	ixEp	=	find(Size == EEG.trials);			% 차원에서의 epoch 위치

	uigeom	=	{ [1 0.5] [1 0.5] [1 0.5] };
	uilist	=	{	...
	{ 'style' 'text' 'string' ['Select one of ' sSzie ' (e.g. 3):'] }	...
			{ 'style' 'edit' 'string' num2str(ixEp) }					...
    { 'style' 'text' 'string' 'Average'}		{'style' 'checkbox' 'value' 0}...
    { 'style' 'text' 'string' 'Concatenate'}	{'style' 'checkbox' 'value' 0}...
				});

	result	=	inputgui(uigeom, uilist,								...
					'pophelp(''pop_loadbv'')', 'Direction for 2-D matrix form');
	if isempty(result), return, end

	ixAvg	= str2num(result{1}); end
	Average	=	str2num(result{2}); end

	Concate	=	userInput{1,7};
