...
if ~isempty(g.timefreqs)							% 위의 조처로, 절대 not empty
	...
	for n=1:tfpoints								% topo 작성
		if n<=plotdim								% tigoum rem: horiz direction
...
%			topoaxes(n)=sbplot(plotdim,plotdim,plotdim*(n+1-plotdim),'ax',curax);
		...
		if strcmpi(g.logfreq, 'off')
			from	=	changeunits(tmpTF,imgax,wholeax);
		else
			from	=	changeunits([tmpTF(1) log(tmpTF(2))],imgax,wholeax);
		end;
%		to			=	changeunits([0.5,0.5],topoaxes(n),wholeax);
		to			=	changeunits([0.5,(TF_TP-1)/10],topoaxes(n),wholeax);
		axes(wholeax);
		OL(n)		=	plot([from(1) to(1)],	[from(2) to(2)],			...
...
%			xlabel(cb, '\muV^2');					% appended by tigoum
%			xlabh	=	get(gca,'XLabel');			% appended by tigoum
%			set(xlabh,'Position', get(xlabh,'Position') - [0 .175 0]); % down
%			set(xlabh,'Position', get(xlabh,'Position') + [0 0 0.2]); % down
%			set(gca,'xaxisLocation','top')
			ylabel(cb, '\muV^2');					% appended by tigoum
			ylabh	=	get(gca,'YLabel');			% appended by tigoum
			set(ylabh,'Position', get(ylabh,'Position') + [0 0.03 0]); % down
			set(ylabh, 'Rot', 0);
		end
		drawnow
	end
...
if g.showchan>0 & ~isempty(g.chanlocs)		% 마우스 event 시 지워지는 현상!
	...
	tcpos	=	get(tc, 'Position');
	axis('square');
	sCh		=	regexprep(g.title, '.*TF \(Ch:([A-Za-z0-9]+)\).*', '$1');
%	ttl		=	title(sCh);	%, 'VerticalAlignment', 'bottom');
	ttl		=	title(['[' sCh ']']);	%, 'VerticalAlignment', 'bottom');
	set(ttl, 'Fontsize', AXES_FONTSIZE - 2);
%	set(ttl, 'Position', [tcpos(1)+tcpos(3)/2 tcpos(2)+tcpos(4) 0]);
end
%{
