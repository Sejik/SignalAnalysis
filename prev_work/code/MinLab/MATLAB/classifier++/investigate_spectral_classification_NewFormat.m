function [ nProc ]	=	investigate_spectral_classification_NewFormat(	...
							hEEG, epos, verbose)
%% FFT & rLDA 기반 classification에 의한 각 subject 별 accuracy 를 산출함.
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
%% band or FOI 를 기준으로 각 대역별로 별도 accuracy 계산해야 더 정확도 높음
%	단, window 추가
eOrg		=	epos;										% 원본 유지 !
fBin		=	hEEG.FreqBins;
...
if length(find( ismember(eOrg{1}.clab, hEEG.ChSide) )) > 0	% EOG 추가 임.
...
try, parfor sj_ix = 1:n_subjects
	...
	% CCA 를 구동하여 AllFOI{B} 에 속한 주파수에 대응하는 매칭 정도 파악
...
%					epos{sj_ix}.fs, epos{sj_ix}.marker, hEEG.COI{B});
	...
	% 다룰 경우, FOI 에는 존재하지만, COI 에는 해당 데이터가 존재치 않게 됨
	fgCOI	=	0;
...
for sj_ix=1:n_subjects			% 30 채널 이상에서 parfor 쓰면 threshing 발생!
...
%	end
	...
	if ~isequal(epo_spec{sj_ix}.t, AllFOI{B})	% 구간 중 일부 요소 제외 형태
...
%					[128/256 100/256 162/256]; [1 1 0] ];	% 5종 색상
	acc		=	[accuracy; accuracy_shuffled; accuracy_side; accuracy_cca; ]';
	labels	=	{'rLDA', 'rLDA(shuffled)', hEEG.ChSide{1}, 'CCA',};
	lColor	=	[	[1 0 0]; [0 0 1]; [0 176/256 80/256];					...
					[128/256 100/256 162/256] ];			% 4종 색상
else
	acc		=	[accuracy; accuracy_shuffled; accuracy_side;				...
				accuracy_svm ]';							% 20 x 4 구조
...
hold on
for ix = 1:size(acc,2), h(ix).FaceColor=lColor(ix,:); end	% 색상 변경
...
if isequal(AllFOI{B}, [AllFOI{B}(1): hEEG.FreqBins :AllFOI{B}(end)])	% 등간격
...
set(gca, 'TickDir','Out', 'TickLength',[0 0])	% tick 경계선을 밖방향, 길이 제거
set(gca, 'yGrid','on');
...
xb			=	bsxfun(@plus, h(1).XData, [h.XOffset]');	%x bar별 중간위치
				% 참고, xb(b, s) 에서, b == bar/sbj, s == sbj
se			=	100 * std(acc) ./ sqrt(n_subjects-1);% 100 환산, 20x4 -> 4
...
%		'-k', 'LineWidth',2);						% 아래 가로선
%	plot([xb(b,s)-xW xb(b,s)+xW],	[yb(b,s)+se(b) yb(b,s)+se(b)],	...
%		'-k', 'LineWidth',2);						% 위 가로선
	plot([xb(b,s)-W xb(b,s)+W],[yb(b,s)+se(b) yb(b,s)+se(b)],'-k');%^가로
	plot([xb(b,s)	xb(b,s)],  [yb(b,s)-se(b) yb(b,s)+se(b)],'-k');%|세로
	plot([xb(b,s)-W xb(b,s)+W],[yb(b,s)-se(b) yb(b,s)-se(b)],'-k');%_가로
end	%for b
end	%for s
...
%h.LineWidth	=	2;
%h.Color		=	'k';
%h.AlignVertexCenters	=	'on';		% 매끄러운 세로선과 가로선
%errorbar_tick(h, .5, 2);
...
%		if b==1, fonts=FontSize; else fonts=6; end			% LDA 만 강조
