function [ nProc ]	=	viz_spectra_NewFormat( hEEG, epos, verbose )
...
%fig_dir = '/home/sven/%s_data/Results/SSVEP_MIN/figs/power_spectra';
%fig_dir = [ hEEG.PATH '/Results/figs/power_spectra'];
fig_dir		=	fullfile(hEEG.Dest, 'power_spectra');
...
parfor sbj_idx=1:n_subjects
	...
	epos{sbj_idx} = load_SSVEP_Min_KU_data_new_format(...
						fullPATH, sbj_list(sbj_idx), cond, verbose);
...
parfor n=1:n_subjects
    ...
    if save_figs
        fname = sprintf('%s__sbj_%02d__power_spectrum', cond, n);
...
if save_figs
    fname = sprintf('%s__grand_average__power_spectrum', cond);
