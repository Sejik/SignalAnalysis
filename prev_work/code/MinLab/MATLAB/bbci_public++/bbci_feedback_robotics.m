function data_feedback= bbci_feedback_robotics(ctrl, data_feedback)
%BBCI_FEEDBACK_ROBOTICS - BBCI Feedback: robot arm dynamics with imagery movement
%
%Synopsis:
% DATA_FEEDBACK= bbci_feedback_robotics(CTRL, DATA_FEEDBACK)
%
%Arguments:
...
%
%Optional Properties:
%	reset: reset robot arm angle to initial status
%	set: register robot arm initial angle
%	print: print out log info. to console window of MATLAB
%{
...
% 위 저자의 개발코드 "bbci_feedback_cursor"를 참고하여, robotics 코드 제작
% Re-Author: tigoum, 2016. 02. 15.

% 일단, 가장 기초적인 구동 코드로서, param은 무시하고, 매 호출마다 로봇 팔의
%	angle을 10씩 증가시키며, 100까지 순환루프 구성
...
%}
  ...
  [DFS.HH, DFS.cfd]= bbci_feedback_robotics_init(opt);
