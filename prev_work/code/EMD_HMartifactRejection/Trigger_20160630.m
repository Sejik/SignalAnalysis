...
for eegFileNum = 1:length(eeg_name)
    ...
    totalTriggerTypeName = strcat('S', num2str(totalTriggerTypeNum(:)));
    ...
    for totalTriggerNum = 1:length(triggerTime)
        ...
        eventSelected = find(cellfun(cellfind(cellstr(strcat('S', num2str(triggerType(totalTriggerNum))))), vmrk.className));
