function [ Head, Common, AllSubject ] = S04sbjlist_AmH(LoadPath, fgSubordination)
...
%clearvars -except LoadPath fgSubordination
	...
	try,	fName	=	ls([ LoadPath '*.dat' ], '-1');	% 하나의 문자열로 올라옴
...
%	fName		=	regexprep(fName, ['^([A-Za-z_]+su[0-9]+).*'], '$1');%이름분리
	...
	Common		=	regexprep(Common,['_(.+)' '_[a-zA-Z0-9]+$'],'$1');	%뒷 이름
%	Common		=	regexprep(Common,['_(.+[^\w])' '[a-zA-Z0-9]+$'],'$1')%뒷 이름
	Common		=	Common( ix );
	[Common, ix, Cnt]	=	unique(sort(Common));	% 중복제거
