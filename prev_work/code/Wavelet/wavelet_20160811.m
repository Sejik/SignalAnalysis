...
% directory
file.inputDir = 'C:\Users\win\Desktop\Research\3. Analyze\Data\VR\6. Export'; % input File directory
...
param.interestLatency{1} = [0.030 0.130]; % N1
param.interestLatency{2} = [0.300 0.500]; % P3
param.zeroLatency = 0.5;
param.m = 7; param.ki = 5; % wavelet parameter
param.fs = 1000;
param.epochLength = 2000;
...
for eegFileNum = 1:length(file.inputFile)
    ...
    for freqNum = 1:length(param.individualFreq)
        ...
        for latencyNum = 1:length(param.interestLatency)
            firstTime = ceil((param.interestLatency{latencyNum}(1)+param.zeroLatency) * param.fs);
            ...
            result.individualFreqMaxAmplitude{freqNum} = shiftdim(squeeze([amplitude, ((latency+firstTime-1) / param.fs -param.zeroLatency)]),1);
        end      
    end    
    cd(file.outputDir);
