...
% directory
file.inputDir = 'D:\research\005.on_going research\002.VR\5.data\1.normal sub\4.after EMD_4500'; % input File directory
file.outputDir = 'D:\research\005.on_going research\002.VR\5.data\1.normal sub\5-1.wavelet_P1N1P3'; % output File directory
...
param.freqStep = 0.5;
param.freqs = 3.5:param.freqStep:70;% wavelet frequency
%param.individualFreq{1} = [0.1 3.5]; % delta
param.individualFreq{1} = [4 7.3]; % theta
param.individualFreq{2} = [8 13]; % alpha
param.individualFreq{3} = [14 30]; % beta
param.individualFreq{4} = [30 70]; % gamma
param.interestLatency{1} = [0.120 0.160]; % P1
param.interestLatency{2} = [0.170 0.200]; % N2
param.interestLatency{3} = [0.360 0.500]; % P3
param.zeroLatency = 0.5;
param.m = 7; param.ki = 5; % wavelet parameter
param.fs = 1000;
param.epochLength = param.fs * 4.5;
...
for eegFileNum = 1:length(eeg_files)
    ...
    file.outputFile{eegFileNum} = file.inputFile{eegFileNum};
end
cd(file.outputDir);
...
for eegFileNum = 1:length(file.inputFile)
    ...
    cd(file.inputDir);
    datData = load(file.inputFile{eegFileNum});
    cnt(:,1,:) = datData.dat;
