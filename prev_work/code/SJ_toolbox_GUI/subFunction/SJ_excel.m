...
%     if isfield(result.statistic, 'latencyAverageWavelet')
%         for interestChanNum = 1:length(param.interestChannel)
%             currentChanNum = cellfun(param.cellfind(param.interestChannel{interestChanNum}), param.wholeChannel);
%             for latencyNum = 1:length(param.interestLatency)
%                 for freqNum = 1:size(result.latencyAverageWavelet, 4)
%                     fileName = strcat('latencyAverageWavelet-',param.interestChannel{interestChanNum}, '-', num2str(latencyNum), '-', param.freqName{freqNum});
                ...
                case 'Wilcoxon-Mann-Whitney-test'
                    ...
                    for subConditionNum = 1:length(statistic{subjectGroupNum, conditionNum}(1).H)
                        ...
                        if isfield(statistic{subjectGroupNum, conditionNum}(1), 'Z')
                            if ~isempty(statistic{subjectGroupNum, conditionNum}(1).Z)
                                T{currentColumn, currentRow+5} = statistic{subjectGroupNum, conditionNum}(1).Z;
                            end
                        end
                ...
                case 'Wilcoxon-Mann-Whitney-test'
                    ...
                    for responseNum = 1:length(statistic{subjectGroupNum, conditionNum}(2).H)
                        ...
                        if isfield(statistic{subjectGroupNum, conditionNum}(2), 'Z')
                            if ~isempty(statistic{subjectGroupNum, conditionNum}(2).Z)
                              T{currentColumn, currentRow+5} = statistic{subjectGroupNum, conditionNum}(2).Z;
                            end
                        end
...
for questionNum = 1:size(result.questionnaire,2)
    ...
    for subConditionNum = 1:size(result.questionnaire, 4)
        T{1, 1+subConditionNum} = param.subCondition{subConditionNum};
    end
    ...
    for questionResponseNum = 1:length(result.questionnaireList{questionNum})
        for subConditionNum = 1:size(result.questionnaire, 4)
            ...
            currentData = raw(logical(currentFile));
            for dataNum = 1:length(currentData)
                T{currentColumn+dataNum, 1} = result.questionnaireList{questionNum}(questionResponseNum);
                T{currentColumn+dataNum, 1+subConditionNum} = currentData(dataNum);
            end
        end
        currentColumn = currentColumn + length(currentData);
    end
    
    currentRow = 3 + size(result.questionnaire, 4);
    T{1, currentRow} = 'questionnaire_statistic';
    T{2, currentRow} = 'RSQ';
    T{3, currentRow} = 'P-value';
    
    currentRow = currentRow + 1;
    for subConditionNum = 1:size(result.questionnaire, 4)
        T{1, currentRow+subConditionNum} = param.subCondition{subConditionNum};
        T{2, currentRow+subConditionNum} = statistic{questionNum}(1).RSQ(subConditionNum);
        T{3, currentRow+subConditionNum} = statistic{questionNum}(1).RSQpValue(subConditionNum);
        currentColumn = currentColumn + 1;
    end
