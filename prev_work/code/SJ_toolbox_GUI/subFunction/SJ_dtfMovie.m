...
param.interestFreq = param.individualFreq{1}(1);

title0 = 'DTF';
for subjectGroupNum = 1:length(result.groupName)
    fprintf('%s: %d/%d processing \n', datestr(now), subjectGroupNum, length(result.groupName));
    for conditionNum = 1:length(param.condition)
        ...
        for freqNum = 1:length(param.freqName)
            ...
            minf = param.individualFreq{freqNum}(1) - param.interestFreq + 1;
            maxf = param.individualFreq{freqNum}(2) - param.interestFreq + 1;
...
roilab = roiData.labels;
roipos = roiData.centers;
vertices = roiData.vertices;
...
opt = struct(  'Channels', 'all',...
    'Whichchannel', [row col],...
    'ValLim', [valmax*param.movieThreshold, 1.0],...
