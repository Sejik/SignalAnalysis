function [] = SJ_granddtfMovie(param, result)
cd(param.outputDir);
    ...
    for conditionNum = 1:length(param.condition)
        for responseNum = 1:size(result.fileGroup, 3)
            for subConditionNum = 1:size(result.fileGroup, 5)
                ...
                grandRaw = mean(result.raw(logical(currentFile),:,:,:,:),1);
                for dtfEpoch = 1:floor((param.epochLength - windowLength + 1)/shiftLength)
                    ...
                    temp.dtf(:, :, :, :, dtfEpoch) = ...
                        SJ_dtf(param, grandRaw(:, (length(param.wholeChannel)-(length(param.roi)-1:-1:0)), startPoint:endPoint));
                end
                grandDTF{responseNum, subConditionNum} = permute(temp.dtf, [1 5 2 3 4]);
                clear temp;
            end
        end
        ...
        for freqNum = 2:length(param.freqName)
            ...
            for dtfEpoch = 1:floor((param.epochLength - windowLength + 1)/shiftLength)
                for responseNum = 1:size(result.fileGroup, 3)
                    for subConditionNum = 1:size(result.fileGroup, 5)
                        dtfData = squeeze(mean(mean(grandDTF{responseNum, subConditionNum}(:, dtfEpoch, minf:maxf, :,:),1, 'omitnan'),3, 'omitnan'));
